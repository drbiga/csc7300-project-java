...
//    }
    ...
    private Node constructTree(String s, Node node) {
        if(!(countLetters(s) <= 2)){
            ...
            node.left = constructTree(sLeft, new Node(sLeft, blockSize));
            node.right = constructTree(sRight, new Node(sRight, blockSize));
...
//                }
//            }
            int count = node.getBlockCount(index, encoding);
